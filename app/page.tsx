...
export default function ImageInpaintingApp() {
  ...
  const createBlackWhiteMask = async (originalImage: HTMLImageElement): Promise<string> => {
    ...
    // Tạm thời xóa ảnh nền để chỉ lấy nét vẽ
    const currentBackground = canvasBackground;
    setCanvasBackground(""); // Xóa ảnh nền
    const maskData = await sketchCanvasRef.current.exportImage("png");
    setCanvasBackground(currentBackground); // Khôi phục ảnh nền
    ...
    // Chuyển thành đen trắng, chỉ giữ vùng vẽ thực sự
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      // Chỉ giữ vùng vẽ (giả sử nét vẽ là trắng tinh khiết từ strokeColor="white")
    ...
    // Lưu mask tạm thời để kiểm tra (tùy chọn)
    const tempLink = document.createElement("a");
    tempLink.href = tempCanvas.toDataURL("image/png");
    tempLink.download = "mask-debug.png";
    tempLink.click();
    tempLink.remove();
  ...
  return (
    <div className="container mx-auto py-8 px-4 font-sans min-h-screen flex flex-col animate-fade-in">
        ...
        <div className="flex flex-col space-y-4">
          <Card className="p-6 flex flex-col gap-6 bg-white rounded-lg shadow-md">
            <div className="flex flex-col gap-2">
              <h2 className="text-xl font-medium text-blue-900">CaslaQuartz AI là ứng dụng thông minh, hỗ trợ lựa chọn đá thạch anh nhân tạo đa dạng, xem trước trực quan và áp dụng dễ dàng vào các dự án nội thất, mang lại sự tiện lợi và chính xác tối ưu.</h2>
              <div
              ...
              >
                ...
                {!image && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <img src="/public/icons/logo2048.jpg" className="h-12 w-12 text-blue-900/50 mb-4" alt="Icon" />
                    ...
                    >
                      <img src="/public/icons/logo2048.jpg" className="h-4 w-4 mr-2" alt="Upload Icon" />
                      Tải ảnh lên
                    </Button>
                    <input
                      type="file"
      ...
      <footer className="mt-12 py-4 text-center text-sm text-blue-900/70">
        <p>Liên hệ: info@caslaquartz.com | Hotline: +84 971 52 53 54 </p>
        <p>© 2025 CaslaQuartz. All rights reserved.</p>
        <p> Ứng dụng và Mô hình AI được phát triển bởi TDNM.</p>
      </footer>
    </div>
  );
}
